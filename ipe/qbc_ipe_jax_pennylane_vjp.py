...
@jax.custom_vjp
def qbc_ipe_vjp(x, y, num_t_wires=8, num_shots=None):
    assert len(x) == len(y)
...
def qbc_ipe_vjp_fwd(x, y, num_t_wires, num_shots):
    return qbc_ipe_vjp(x, y, num_t_wires, num_shots), (y, x)
...
qbc_ipe_vjp.defvjp(qbc_ipe_vjp_fwd, qbc_ipe_vjp_bwd)
...
#     primal_out = qbc_ipe_vjp(primal0, primal1)
#     tangent_out_1 = qbc_ipe_vjp(primal1, vector0_dot)
#     tangent_out_2 = qbc_ipe_vjp(primal0, vector1_dot)
...
W_grad = jax.jacrev(loss, argnums=0)(W, b)
print("b_grad = ", b_grad)
print("W_grad = ", W_grad)
...
# x =  [ 0.52  0.06 -1.3   0.51]
# x =  [ 0.74 -2.49  1.39  0.62]
# b_grad =  -1.668494062179321
# W_grad =  [-1.42820196  3.11378908 -2.68983649 -0.11220568]

...
def test_qbc_ipe_algorithm(n, num_t_wires):
    a = -5.0
    b = 5.0
    x = (b - a) * np.random.random(2**n) + a
    y = (b - a) * np.random.random(2**n) + a
    rho, _, _, _, _, _ = qbc_ipe_algorithm(x, y, num_t_wires)
    assert math.isclose(rho, np.inner(x, y), rel_tol=1e-1 * (b - a))

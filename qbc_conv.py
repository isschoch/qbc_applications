...
def qbc_conv_algorithm(x, y, k, num_t_wires=10):
    ...
    def U_y():
        for one_idx in y_indices:
            qml.ctrl(qml.PauliX, control=c_wires, control_values=one_idx)(wires=o_wires[1])    
    ...
    def U_minus():
        ...
        for (n_idx, n_wire) in enumerate(n_wires):
            qml.ctrl(add_k_fourier, control=n_wire)(2**(num_n_wires - n_idx - 1), wires=c_wires)
    ...
    def qbc(U_x, U_y, U_minus):
        ...
        qml.QuantumPhaseEstimation(my_unitary, target_wires=range(num_t_wires, num_t_wires + num_n_wires + num_c_wires + 2), estimation_wires=t_wires)
